...
public class GameOfLife {
	...
	public static void test2(String fileName) {
		...
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[0].length - 1; j++) {
				int tmpVal = cellValue(board, i, j);
				test[i][j] = tmpVal;
			}
		}
		print(test);
	...
	public static int[][] read(String fileName) {
		...
		// Loops while the file doesnt empty
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		int outValue = cellValue;
		int neighborsNum = count(board, i, j);
		if (cellValue == 1) {
			if (neighborsNum == 2 || neighborsNum == 3) {
				outValue = 1;
			} else outValue = 0;
		} else {
			if (neighborsNum == 3) outValue = 1;
		}
		return outValue;
	}
	...
	public static int count(int[][] board, int i, int j) {
		int neighborsNum = 0;
		if (board[i + 1][j] == 1) neighborsNum++;
		if (board[i + 1][j + 1] == 1) neighborsNum++;
		if (board[i][j + 1] == 1) neighborsNum++;
		if (board[i - 1][j + 1] == 1) neighborsNum++;
		if (board[i - 1][j] == 1) neighborsNum++;
		if (board[i - 1][j - 1] == 1) neighborsNum++;
		if (board[i][j - 1] == 1) neighborsNum++;
